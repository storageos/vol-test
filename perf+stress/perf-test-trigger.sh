...
for IaaS in $IAAS; do
    ...
    JOBUID="${PROFILE}-$(echo $STORAGEOS_VERSION | tr '.' '-')-$BUILD_TAG"
      ...
      env LOG_USER=$LOG_USER LOG_PASS=$LOG_PASS DO_TOKEN=$DO_TOKEN PVTK_PATH=$PVTK_PATH PUBK_PATH=$PUBK_PATH JOBUID=$JOBUID STORAGEOS_VERSION=$STORAGEOS_VERSION $IAASDIR/scripts/new-perf-cluster.sh 
